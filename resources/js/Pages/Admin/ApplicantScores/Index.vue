...
const average = (applicant: any) => {
  ...
  return (values.reduce((total, value) => total + Number(value), 0) / 4).toFixed(2);
};
  ...
  <main class="min-h-screen p-5 md:p-8">
    <section class="mx-auto max-w-7xl">
      <div>
        ...
        <p class="mt-2 text-sm text-slate-500">Daftar pendaftar yang sudah dinyatakan diterima oleh admin.</p>
      </div>
        ...
        <table class="w-full min-w-[980px] text-left text-sm">
          ...
          <tbody>
            <tr v-for="applicant in applicants.data" :key="applicant.id" class="border-b border-slate-100 hover:bg-emerald-50/40">
              ...
              <td class="p-4 text-center"><span class="rounded-lg bg-amber-50 px-3 py-2 font-extrabold text-amber-800">{{ average(applicant) }}</span></td>
