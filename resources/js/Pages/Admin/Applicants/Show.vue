...
const canReview = computed(() => ['super_admin', 'admin_pmb', 'reviewer'].includes(page.props.auth.user.role));
...
<template>
  ...
  <main class="applicant-detail-page min-h-screen px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
    <section class="relative z-10 mx-auto max-w-[1380px] space-y-4">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          <a v-if="applicant.selection_card_url" :href="applicant.selection_card_url" class="action-btn bg-emerald-50 text-emerald-900 shadow-lg shadow-black/10 hover:bg-white"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3h9l4 4v14H6zM15 3v5h5M9 12h6M9 16h6" stroke-linecap="round" stroke-linejoin="round"/></svg>Kartu Seleksi</a>
            ...
            <div v-if="applicant.documents.length" class="mt-3 divide-y divide-slate-200/80">
              <div v-for="d in applicant.documents" :key="d.id" class="document-row">
                ...
                <div class="grid w-full shrink-0 grid-cols-2 gap-1.5 sm:flex sm:w-auto sm:flex-wrap sm:justify-end">
                  ...
                  <button v-if="canReview" type="button" :disabled="reviewingId === d.id" class="mini-btn text-emerald-700" :aria-label="`Terima ${documentNames[d.type] || d.type}`" @click="review(d,'valid')"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.4" aria-hidden="true"><path d="m5 12 4.5 4.5L19 7" stroke-linecap="round" stroke-linejoin="round"/></svg><span>{{ reviewingId === d.id ? 'Menyimpan…' : 'Terima' }}</span></button>
                  <button v-if="canReview" type="button" :disabled="reviewingId === d.id" class="mini-btn text-amber-700" :aria-label="`Minta revisi ${documentNames[d.type] || d.type}`" @click="review(d,'revision_required')"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="M12 8v5m0 4h.01M10.3 4.8 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3l-7.5-13.2a2 2 0 0 0-3.4 0Z" stroke-linecap="round" stroke-linejoin="round"/></svg><span>{{ reviewingId === d.id ? 'Menyimpan…' : 'Minta Revisi' }}</span></button>
                </div>
              </div>
            </div>
            <div v-else class="mt-4 rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Belum ada dokumen yang diunggah.</div>
          </section>

          <aside class="detail-panel"><h2 class="panel-title"><span class="panel-icon"><svg viewBox="0 0 24 24"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg></span>Status Proses</h2><div class="mt-4 space-y-3"><div v-for="item in [{ label: 'Pembayaran', value: applicant.payment_status, note: applicant.payment_status === 'paid' ? 'Pembayaran telah dikonfirmasi.' : 'Pembayaran belum dilakukan atau belum berhasil dikonfirmasi.' }, { label: 'Berkas', value: applicant.document_status, note: applicant.document_status === 'complete' ? 'Berkas dinyatakan lengkap.' : 'Berkas sedang dalam peninjauan.' }, { label: 'Seleksi', value: applicant.selection_status, note: applicant.selection_status === 'passed' ? 'Pendaftar dinyatakan lulus.' : 'Jadwal dan hasil akan diperbarui.' }]" :key="item.label" class="process-card"><div class="flex items-center justify-between gap-2"><b class="text-sm text-emerald-950">{{ item.label }}</b><span class="rounded-full px-2 py-0.5 text-[10px] font-bold ring-1 ring-inset" :class="badge(item.value)[1]">{{ badge(item.value)[0] }}</span></div><p class="mt-1 text-xs leading-5 text-slate-500">{{ item.note }}</p><dl v-if="item.label === 'Seleksi' && applicant.selection_schedule" class="mt-3 grid gap-2 rounded-lg border border-blue-100 bg-blue-50/60 p-3 text-xs"><div class="flex justify-between gap-3"><dt class="text-slate-500">Tanggal</dt><dd class="text-right font-bold text-slate-700">{{ applicant.selection_schedule.date }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-500">Waktu</dt><dd class="text-right font-bold text-slate-700">{{ applicant.selection_schedule.time }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-500">Lokasi</dt><dd class="max-w-[65%] text-right font-bold text-slate-700">{{ applicant.selection_schedule.location }}</dd></div></dl><a v-if="item.label === 'Seleksi' && applicant.selection_card_url" :href="applicant.selection_card_url" class="mt-3 flex items-center justify-center gap-2 rounded-lg bg-emerald-800 px-3 py-2 text-xs font-black text-white transition hover:bg-emerald-700">↓ Download Kartu Seleksi</a></div></div></aside>
        </div>
    ...
    <div v-if="selectedDocument" class="document-modal" role="dialog" aria-modal="true" aria-label="Pratinjau dokumen" @click.self="selectedDocument = null">
      <section class="document-modal-card">
        <header class="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div><p class="text-[10px] font-bold uppercase tracking-[.14em] text-emerald-700">Pratinjau Dokumen</p><h2 class="mt-1 text-lg font-black text-emerald-950">{{ documentNames[selectedDocument.type] || selectedDocument.type }}</h2></div>
          <button type="button" class="modal-close" aria-label="Tutup pratinjau" @click="selectedDocument = null"><svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 6 12 12M18 6 6 18" stroke-linecap="round"/></svg></button>
        </header>
        <div class="document-modal-body">
          <iframe v-if="String(selectedDocument.mime_type || '').toLowerCase() === 'application/pdf'" :src="`/admin/documents/${selectedDocument.id}/view`" title="Pratinjau PDF" class="h-[70vh] w-full rounded-lg border border-slate-200 bg-white" />
          <img v-else :src="`/admin/documents/${selectedDocument.id}/view`" :alt="documentNames[selectedDocument.type] || selectedDocument.type" class="max-h-[70vh] max-w-full rounded-lg object-contain" />
        </div>
      </section>
    </div>
  </main>
...
.document-row { @apply flex flex-col items-start justify-between gap-3 py-3 sm:flex-row sm:items-center; }
.mini-btn { @apply inline-flex min-h-8 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-black transition hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 disabled:cursor-wait disabled:opacity-40; }
.document-modal { @apply fixed inset-0 z-50 grid place-items-center bg-emerald-950/70 p-4 backdrop-blur-sm; }
.document-modal-card { @apply w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl; }
.document-modal-body { @apply grid max-h-[calc(100vh-7rem)] min-h-[240px] place-items-center overflow-auto bg-slate-50 p-4; }
.modal-close { @apply grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600; }
