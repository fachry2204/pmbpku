    ...
    <section class="mx-auto max-w-6xl space-y-6">
      ...
      <article class="overflow-hidden rounded-3xl bg-white shadow-xl shadow-emerald-950/5 ring-1 ring-slate-200/80">
        <header class="relative overflow-hidden bg-gradient-to-br from-emerald-950 via-emerald-800 to-teal-700 px-6 py-8 text-white sm:px-9">
          ...
          <div class="relative flex flex-col gap-5 pr-0 sm:flex-row sm:items-center" :class="photoDocument ? 'lg:pr-44' : ''"><div class="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-white/15 text-2xl font-black shadow-inner ring-1 ring-white/20 backdrop-blur"><img v-if="photoDocument" :src="`/admin/documents/${photoDocument.id}/view`" :alt="`Pas foto ${applicant.full_name}`" class="h-full w-full object-cover"/><span v-else>{{ applicant.full_name?.charAt(0) }}</span></div><div class="min-w-0 flex-1"><p class="font-mono text-sm font-semibold tracking-wide text-emerald-100">{{ applicant.registration_number }}</p><h1 class="mt-1 truncate text-2xl font-black tracking-tight sm:text-4xl">{{ applicant.full_name }}</h1><p class="mt-2 text-sm text-emerald-100">Detail lengkap dan progres pendaftaran calon peserta.</p></div><div v-if="photoDocument" class="mt-2 hidden h-44 w-32 shrink-0 overflow-hidden rounded-2xl bg-white/10 shadow-2xl ring-4 ring-white/20 lg:absolute lg:right-3 lg:top-1/2 lg:block lg:-translate-y-1/2"><img :src="`/admin/documents/${photoDocument.id}/view`" :alt="`Pas foto ${applicant.full_name}`" class="h-full w-full object-cover transition duration-300 hover:scale-105"/></div></div>
        </header>
        <div class="grid gap-7 p-6 sm:p-9 lg:grid-cols-[1fr_320px]">
          <section><h2 class="section-title">Informasi Pribadi</h2><div class="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div class="info-tile"><span class="icon-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M8 3v3m8-3v3M4 9h16M5 5h14v16H5z"/></svg></span><div><p class="info-label">Tempat, tanggal lahir</p><p class="info-value">{{ applicant.birth_place }}, {{ formatDate(applicant.birth_date) }}</p></div></div>
