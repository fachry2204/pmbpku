    ...
    <section class="relative z-10 mx-auto max-w-[1120px] space-y-4">
      ...
      <article class="overflow-hidden rounded-[28px] border border-white/70 bg-[#fcfdfb] shadow-2xl shadow-black/25">
        ...
        <div class="grid gap-4 p-5 sm:p-7 lg:grid-cols-2">
          ...
          <section class="detail-panel"><div class="flex items-center justify-between"><h2 class="panel-title"><span class="panel-icon"><svg viewBox="0 0 24 24"><path d="M6 3h9l4 4v14H6zM15 3v5h5M9 12h6M9 16h5"/></svg></span>Dokumen Pendaftaran</h2><span class="text-xs font-bold text-emerald-700">{{ applicant.documents.length }} file</span></div><div v-if="applicant.documents.length" class="mt-3 divide-y divide-slate-200/80"><div v-for="d in applicant.documents" :key="d.id" class="document-row"><div class="min-w-0"><p class="truncate text-sm font-bold text-slate-700">{{ documentNames[d.type] || d.type }}</p><span class="mt-1 inline-flex rounded-md px-2 py-0.5 text-[10px] font-bold ring-1 ring-inset" :class="badge(d.verification_status)[1]">{{ badge(d.verification_status)[0] }}</span></div><div class="flex shrink-0 gap-1"><button type="button" class="mini-btn text-sky-700" title="Lihat dokumen" :aria-label="`Lihat ${documentNames[d.type] || d.type}`" @click="selectedDocument = d"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/></svg></button><a :href="`/admin/documents/${d.id}/download`" class="mini-btn" title="Unduh dokumen" aria-label="Unduh dokumen"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 20h14" stroke-linecap="round" stroke-linejoin="round"/></svg></a><button type="button" :disabled="reviewingId === d.id" class="mini-btn text-emerald-700" title="Tandai valid" aria-label="Tandai valid" @click="review(d,'valid')"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.4"><path d="m5 12 4.5 4.5L19 7" stroke-linecap="round" stroke-linejoin="round"/></svg></button><button type="button" :disabled="reviewingId === d.id" class="mini-btn text-amber-700" title="Minta perbaikan" aria-label="Minta perbaikan" @click="review(d,'revision_required')"><svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 8v5m0 4h.01M10.3 4.8 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3l-7.5-13.2a2 2 0 0 0-3.4 0Z" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div></div></div><div v-else class="mt-4 rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Belum ada dokumen yang diunggah.</div></section>
