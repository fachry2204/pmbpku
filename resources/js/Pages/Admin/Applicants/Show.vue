...
<template>
  ...
  <main class="applicant-detail-page min-h-screen px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
    <section class="relative z-10 mx-auto max-w-[1280px] space-y-5">
      ...
      <article class="overflow-hidden rounded-[28px] border border-white/70 bg-[#fcfdfb] shadow-2xl shadow-black/25">
        <header class="profile-header grid gap-6 px-6 py-7 sm:px-8 lg:grid-cols-[190px_1fr] lg:px-10">
          <div class="mx-auto h-[236px] w-[176px] overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-700 to-emerald-950 shadow-xl ring-1 ring-emerald-950/20 lg:row-span-2"><img v-if="photoDocument" :src="`/admin/documents/${photoDocument.id}/view`" :alt="`Pas foto ${applicant.full_name}`" class="h-full w-full object-cover transition duration-500 hover:scale-105"/><span v-else class="grid h-full w-full place-items-center text-6xl font-black text-white/90">{{ applicant.full_name?.charAt(0) }}</span></div>
          <div class="self-end text-center lg:text-left"><p class="text-xs font-bold uppercase tracking-[.12em] text-emerald-900/65">Nomor Pendaftaran</p><p class="mt-1 font-mono text-lg font-black text-emerald-700">{{ applicant.registration_number }}</p><h1 class="mt-1 text-3xl font-black tracking-tight text-emerald-950 sm:text-4xl lg:text-5xl">{{ applicant.full_name }}</h1></div>
          <div class="status-banner grid gap-5 rounded-2xl border border-amber-300 bg-gradient-to-r from-[#fff4d5] via-[#fff9e9] to-[#ffefbf] p-5 lg:grid-cols-[260px_1fr] lg:items-center"><div class="flex items-center gap-4"><span class="grid h-16 w-16 shrink-0 place-items-center rounded-full border border-amber-300 bg-white/70 text-amber-700 shadow-sm"><svg viewBox="0 0 24 24" class="h-8 w-8" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M6 3h9l4 4v14H6zM15 3v5h5M9 12h6M9 16h4" stroke-linecap="round" stroke-linejoin="round"/></svg></span><div><p class="text-sm font-semibold text-amber-950/70">Status Saat Ini</p><p class="text-2xl font-black text-amber-950">{{ currentStatus }}</p></div></div><p class="border-amber-300 text-sm leading-6 text-amber-950/80 lg:border-l lg:pl-6">{{ statusDescription }}</p></div>
        </header>

        <div class="mx-5 rounded-2xl border border-slate-200 bg-white px-4 py-6 shadow-lg shadow-slate-900/5 sm:mx-8 sm:px-7">
        ...
        <div class="grid gap-5 p-5 sm:p-8 xl:grid-cols-[1fr_1.15fr_.9fr]">
          <section class="detail-panel"><h2 class="panel-title"><span class="panel-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/></svg></span>Data Calon Mahasiswa</h2><dl class="mt-3 divide-y divide-slate-200/80"><div class="data-row"><dt>Nama Lengkap</dt><dd>{{ applicant.full_name }}</dd></div><div class="data-row"><dt>Tempat, Tanggal Lahir</dt><dd>{{ applicant.birth_place }}, {{ formatDate(applicant.birth_date) }}</dd></div><div class="data-row"><dt>Email</dt><dd><a :href="`mailto:${applicant.email}`">{{ applicant.email }}</a></dd></div><div class="data-row"><dt>Nomor WhatsApp</dt><dd><a :href="`https://wa.me/${applicant.whatsapp_normalized}`" target="_blank">{{ applicant.whatsapp_display }}</a></dd></div><div class="data-row"><dt>Alamat</dt><dd class="whitespace-pre-line">{{ applicant.address }}</dd></div></dl></section>
        ...
        <footer class="mx-5 mb-5 flex items-center gap-4 rounded-2xl border border-emerald-900/10 bg-emerald-50/70 px-5 py-4 sm:mx-8 sm:mb-8"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-emerald-800 shadow-sm"><svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M7 10V7a5 5 0 0 1 10 0v3M5 10h14v11H5zM12 14v3" stroke-linecap="round" stroke-linejoin="round"/></svg></span><div><p class="text-sm font-black text-emerald-950">Data Pendaftar Terlindungi</p><p class="mt-0.5 text-xs leading-5 text-slate-600">Gunakan informasi pribadi dan dokumen hanya untuk proses administrasi dan seleksi PMB Pendidikan Kader Ulama.</p></div></footer>
      </article>
    </section>
  </main>
</template>
...
.action-btn { @apply inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition duration-200 hover:-translate-y-0.5 active:translate-y-0 focus:outline-none focus:ring-2 focus:ring-amber-300 focus:ring-offset-2 focus:ring-offset-emerald-950; }
.detail-panel { @apply min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-md shadow-slate-900/5; }
.panel-title { @apply flex items-center gap-2.5 text-base font-black text-emerald-950; }
.panel-icon { @apply grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-emerald-50 text-emerald-800; }
.panel-icon svg { @apply h-5 w-5; fill: none; stroke: currentColor; stroke-width: 1.8; stroke-linecap: round; stroke-linejoin: round; }
.data-row { @apply grid gap-1 py-3 text-sm sm:grid-cols-[138px_1fr]; }
...
.document-row { @apply flex items-center justify-between gap-3 py-3; }
.mini-btn { @apply grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-xs font-black text-slate-600 transition hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 disabled:opacity-40; }
.process-card { @apply rounded-xl border border-slate-200 bg-slate-50/70 p-4 transition duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-white hover:shadow-md; }
