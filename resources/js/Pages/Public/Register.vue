  ...
  <main class="islamic-gradient-page min-h-screen px-4 py-10">
    <section class="mx-auto max-w-4xl">
      ...
      <form @submit.prevent="submit" class="islamic-glass-card rounded-[28px] p-6 md:p-10">
        <header class="mb-8 border-b pb-6"><p class="text-sm font-bold uppercase tracking-[.2em] text-[#b38b21]">Langkah {{step}} dari {{totalSteps}}</p><h1 class="mt-2 text-3xl font-extrabold text-[#064e3b]">{{step===1?'Data Diri Pendaftar':props.documentUploadEnabled&&step===2?'Unggah Dokumen':'Konfirmasi Pendaftaran'}}</h1><p v-if="step < finalStep" class="mt-2 text-slate-500">{{step===1?'Pastikan identitas dan kontak dapat dihubungi.':'Format JPG, PNG, atau PDF; maksimal 5 MB per file.'}}</p></header>
        ...
        <footer class="mt-8 flex items-center justify-between border-t pt-6"><button v-if="step>1" type="button" @click="step--" class="rounded-xl border border-slate-300 px-6 py-3 font-bold text-slate-700">← Kembali</button><span v-else></span><button v-if="step<finalStep" type="button" @click="next" :disabled="step===1?!dataReady:!docsReady" class="rounded-xl bg-[#064e3b] px-7 py-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Lanjutkan →</button><button v-else type="submit" :disabled="form.processing||!form.consent" class="rounded-xl bg-[#d4af37] px-7 py-3 font-extrabold text-[#173b2e] disabled:cursor-not-allowed disabled:opacity-40">{{form.processing?'Menyimpan…':'Simpan Pendaftaran →'}}</button></footer>
      </form>
    </section>
  </main>
</template>
