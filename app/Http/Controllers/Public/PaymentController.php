...
class PaymentController extends Controller
{
    ...
    public function show(Request $request, string $registrationNumber, DuitkuClient $duitku, SettingsService $settings): Response
    {
        ...
        try {
            ...
            $channels = Cache::remember('duitku.channels.'.config('services.duitku.mode').'.'.$amount, 300, fn () => $duitku->channels($amount));
        } catch (\Throwable) {
    ...
    public function create(Request $request, string $registrationNumber, DuitkuClient $duitku, SettingsService $settings): RedirectResponse
    {
        ...
        $amount = $this->amount($applicant, $settings);
        $remote = $duitku->create($applicant, $data['method'], $merchantRef, $amount);
        $payment = DB::transaction(fn () => Payment::create([
            'applicant_id' => $applicant->id, 'provider' => 'duitku', 'merchant_ref' => $merchantRef,
            'provider_reference' => $remote['reference'], 'payment_method' => $remote['paymentMethod'] ?? $data['method'],
            'base_amount' => $amount, 'fee_merchant' => (int) ($remote['fee_merchant'] ?? 0),
            'fee_customer' => 0, 'total_amount' => (int) ($remote['amount'] ?? $amount),
            'status' => 'unpaid', 'checkout_url' => $remote['paymentUrl'] ?? null,
            'instructions_json' => isset($remote['vaNumber']) ? ['va_number' => $remote['vaNumber']] : null,
