...
class RegistrationController extends Controller {
 public function create(DuitkuClient $duitku, SettingsService $settings):Response{$channels=[];$paymentError=null;$amount=(int)$settings->get('pmb.registration_fee',250000);try{$channels=Cache::remember('duitku.channels.'.config('services.duitku.mode').'.'.$amount,300,fn()=>$duitku->channels($amount));}catch(\Throwable){$paymentError='Channel Duitku belum tersedia. Admin perlu melengkapi credential sandbox.';}return Inertia::render('Public/Register',['channels'=>$channels,'paymentError'=>$paymentError]);}
