...
class RegistrationFlowTest extends TestCase
{
    ...
    public function test_registration_stores_private_files_and_queues_notifications(): void
    {
        Storage::fake('local');
        Queue::fake();
        Setting::create(['group' => 'pmb', 'key' => 'pmb.registration_year', 'value' => '2030', 'type' => 'integer', 'is_encrypted' => false]);
        ...
        foreach ($a->documents as $document) {
            ...
            $this->assertStringStartsWith($a->storageDirectory().'/', $document->path);
            $this->assertStringNotContainsString($document->original_name, $document->path);
        }Queue::assertPushed(SendApplicantNotification::class, 2);
    }
}
