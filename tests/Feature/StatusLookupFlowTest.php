...
class StatusLookupFlowTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_status_can_be_found_by_email_without_otp(): void
    {
        ...
        $this->post('/cek-status', ['identifier' => strtoupper($applicant->email)])->assertRedirect(route('status.show'))->assertSessionHas('status_applicant_id', $applicant->id);
    }

    public function test_status_can_be_found_by_phone_without_otp(): void
    {
        $applicant = $this->applicant();
        $this->post('/cek-status', ['identifier' => '0812 3456 7890'])->assertRedirect(route('status.show'))->assertSessionHas('status_applicant_id', $applicant->id);
    }

    public function test_status_can_be_found_by_registration_number(): void
    {
        $applicant = $this->applicant();
        $this->post('/cek-status', ['identifier' => '  pku-2026-000009  '])->assertRedirect(route('status.show'))->assertSessionHas('status_applicant_id', $applicant->id);
    }
    ...
    public function test_signed_email_link_opens_applicant_detail_without_lookup(): void
    {
        ...
        $this->get($url)->assertRedirect(route('status.show'))->assertSessionHas('status_applicant_id', $applicant->id);
    ...
    {
        $this->applicant();
        $this->post('/cek-status', ['identifier' => 'wrong@example.test'])->assertSessionHasErrors('identifier');
    }
}
