...
class AdminOperationsTest extends TestCase
{
    ...
    {
        $p = AdmissionPeriod::create(['name' => 'Tes '.$selection, 'year' => random_int(2026, 2099), 'registration_prefix' => 'PKU', 'starts_at' => now()->subDay(), 'ends_at' => now()->addDay(), 'registration_fee' => 250000, 'is_active' => true]);
    ...
    {
        $admin = User::factory()->create(['username' => 'admin_test', 'role' => 'super_admin', 'is_active' => true]);
        $this->actingAs($admin)->patch("/admin/users/{$admin->id}", ['name' => $admin->name, 'username' => $admin->username, 'role' => 'super_admin', 'is_active' => false])->assertSessionHasErrors('role');
    ...
    public function test_score_page_uses_configured_weights_for_final_score(): void
    {
        $admin = User::factory()->create(['role' => 'admin_pmb', 'is_active' => true]);
        $passed = $this->applicant();
        $passed->score()->create(['score_1' => 80, 'score_2' => 90, 'score_3' => 70, 'score_4' => 100, 'updated_by' => $admin->id]);
        $this->actingAs($admin)->get('/admin/applicant-scores')->assertOk()->assertInertia(fn (Assert $page) => $page->where('scoreWeights', [25, 10, 50, 15])->where('applicants.data.0.final_score', 79));
    }

    public function test_scores_are_rejected_for_applicant_not_accepted(): void
    {
        $admin = User::factory()->create(['role' => 'admin_pmb', 'is_active' => true]);
        $applicant = $this->applicant('not_passed');
        $this->actingAs($admin)->patch("/admin/applicant-scores/{$applicant->id}", ['score_1' => 80, 'score_2' => 80, 'score_3' => 80, 'score_4' => 80])->assertStatus(422);
        $this->assertDatabaseCount('applicant_scores', 0);
    }

    public function test_viewer_cannot_manage_scores(): void
    {
        $viewer = User::factory()->create(['role' => 'viewer', 'is_active' => true]);
        $this->actingAs($viewer)->get('/admin/applicant-scores')->assertForbidden();
    }
}
